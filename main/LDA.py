...
def lda_analyze(id, posdata, negdata):
    # # 载入情感分析后的数据
    # posdata = pd.read_csv("posdata.csv", encoding='utf-8')
    # negdata = pd.read_csv("negdata.csv", encoding='utf-8')

    # 只选择形容词和名词
    posdata = posdata[(posdata['nature'] == 'n') | (posdata['nature'] == 'a')]
    negdata = negdata[(negdata['nature'] == 'n') | (negdata['nature'] == 'a')]
    # posdata = posdata[posdata['word'].apply(len) != 1]
    # negdata = negdata[negdata['word'].apply(len) != 1]
    posdata = posdata['word']
    negdata = negdata['word']
    # print(posdata,negdata)
    from gensim import corpora, models

    p = [[i] for i in posdata]

    # 建立词典
    pos_dict = corpora.Dictionary([[i] for i in posdata])  # 正面
    neg_dict = corpora.Dictionary([[i] for i in negdata])  # 负面

    # 建立语料库
    pos_corpus = [pos_dict.doc2bow(j) for j in [[i] for i in posdata]]  # 正面
    neg_corpus = [neg_dict.doc2bow(j) for j in [[i] for i in negdata]]  # 负面
    # a = set()
    # for i in pos_corpus:
    #     if i[0] not in a:
    #         a.add(i[0])
